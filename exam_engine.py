...
class ExamModuleEngine:
    ...
    def __init__(self, module_name: str, questions: List[Dict[str, Any]]):
        ...
        """
        if len(questions) < self.QUESTIONS_AND_TIMES_PER_MODULE.get(
            module_name, self.DEFAULT_VALUE
        ):
            raise ValueError(
        ...
        self.questions = random.sample(
            questions,
            self.QUESTIONS_AND_TIMES_PER_MODULE.get(module_name, self.DEFAULT_VALUE),
        )
    ...
    def save_current_answer(self, answer: str):
        ...
        cod_domanda = current_question.get(
            "cod_domanda", str(self.current_question_idx)
        )
    ...
    def finish_module(self) -> ModuleResult:
        ...
        score_percentage = (
            (correct / len(self.questions) * 100) if len(self.questions) > 0 else 0
        )
...
class ExamEngine:
    ...
    def __init__(self, modules_data: Dict[str, List[Dict[str, Any]]]):
        ...
        for module_name, questions in modules_data.items():
            try:
                ...
                display_name = (
                    module_name.replace("_final.json", "").replace("_", " ").title()
                )
    ...
    def get_exam_progress(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "current_module_name": (
                self.get_current_module().module_name
