...
def renumber_questions(quiz_file: str):
    """
    Riassegna i num_domanda in modo consecutivo da 1 a N
    Utile se si cancellano manualmente domande dal JSON

    Args:
        quiz_file: nome del file JSON da caricare

    Returns:
        bool indicante se ci sono stati aggiornamenti
    """
    file_path = f"QUIZ_CLEAN/JSON/{quiz_file}"
    ...
    # Riassegna i num_domanda in modo consecutivo
    for idx, question in enumerate(data, start=1):
        if question.get("num_domanda") != idx:
            question["num_domanda"] = idx
            updated = True
...
def main():
    ...
    for filename in json_files:
        ...
        # Riassegna i num_domanda in modo consecutivo
        print(f"  Renumbering questions...")
        renumbered = renumber_questions(filename)
        if renumbered:
            print(f"  ✓ {filename} renumbered successfully")
        else:
            print(f"  ✓ {filename} already has consecutive numbering")
